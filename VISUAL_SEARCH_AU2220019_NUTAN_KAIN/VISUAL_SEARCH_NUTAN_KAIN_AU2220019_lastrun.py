...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Lenovo\\Documents\\MON 2024-25\\FULL_TERM\\PSY-310_LAB_IN_PSYCHOLOGY\\5-4_OCT_VISUAL_SEARCH_TASK\\VISUAL_SEARCH_NUTAN_KAIN_AU2220019_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    guideline = visual.TextStim(win=win, name='guideline',
        text="Hope you're doing well,\nIn this task, you have to find the letter 'T' with the mouse and click on it. \nIf you would like to participate, please press 'SPACE'\nthank you",
        font='Cooper Black',
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    guidelinesComponents = [guideline, key_resp]
    ...
    while continueRoutine:
        ...
        # if guideline is starting this frame...
        if guideline.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            guideline.frameNStart = frameN  # exact frame index
            guideline.tStart = t  # local t and not account for scr refresh
            guideline.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(guideline, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'guideline.started')
            # update status
            guideline.status = STARTED
            guideline.setAutoDraw(True)
        
        # if guideline is active this frame...
        if guideline.status == STARTED:
            # update params
            pass
    ...
    for thisTrial in trials:
        ...
        # keep track of which components have finished
        trialComponents = [mouse, polygon, target]
