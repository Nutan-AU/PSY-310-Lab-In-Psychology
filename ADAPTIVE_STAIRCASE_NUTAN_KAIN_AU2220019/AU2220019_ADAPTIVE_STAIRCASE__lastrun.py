...
expName = '23_AUG_ADAP_STAIR'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Lenovo\\Documents\\MON 2024-25\\FULL_TERM\\PSY-310_LAB_IN_PSYCHOLOGY\\3-23RD_AUGUST_ADAPTIVE_STAIRS\\AU2220019_ADAPTIVE_STAIRCASE__lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    image_2 = visual.ImageStim(
        win=win,
        name='image_2', 
        image='C:/Users/Lenovo/Pictures/Screenshots/Screenshot 2024-09-06 112504.png', mask=None, anchor='center',
    ...
    text = visual.TextStim(win=win, name='text',
        text='This experiment invovles you to respond to certain stimuli orientations presented on the screen.\nFor participating, please read the guidlines below.\nPress the following keys\n1. Z= if the orientation is towards the left\n2. M= if the orientation is towards the right\nIf you have reached here and would like to participate, press SPACE to begin.',
        font='Cooper Black',
    ...
    fixation = visual.ShapeStim(
        ...
        lineWidth=1.0,     colorSpace='rgb',  lineColor=[0.9608, 0.9608, 0.6471], fillColor=[-1.0000, -1.0000, -1.0000],
        opacity=None, depth=-1.0, interpolate=True)
    grating = visual.GratingStim(
        win=win, name='grating',
        tex='sin', mask='gauss', anchor='center',
        ori=1.0, pos=(0, 0), size=(0.3, 0.3), sf=8.0, phase=0.0,
        color=[0.9216, 0.9216, 0.7255], colorSpace='rgb',
        opacity=None, contrast=0.5, blendmode='avg',
        texRes=128.0, interpolate=True, depth=-2.0)
    key_resp_2 = keyboard.Keyboard(deviceName='key_resp_2')
    
    # --- Initialize components for Routine "salutations" ---
    text_5 = visual.TextStim(win=win, name='text_5',
        text='Thank you for your participation. \n',
        font='Cooper Black',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color=[0.6471, 0.4118, 0.0980], colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    ...
    # --------Prepare to start Staircase "scase" --------
    # set up handler to look after next chosen value etc
    scase = data.StairHandler(startVal=10.0, extraInfo=expInfo,
        stepSizes=[2,1,1,0.5], stepType='lin',
        nReversals=0.0, nTrials=100.0, 
        nUp=1.0, nDown=3.0,
        minVal=1.0, maxVal=20.0,
        originPath=-1, name='scase')
    thisExp.addLoop(scase)  # add the loop to the experiment
    level = thisScase = 10.0  # initialise some vals
    
    for thisScase in scase:
        currentLoop = scase
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('staircase.started', globalClock.getTime(format='float'))
        ...
        _key_resp_2_allKeys = []
        # keep track of which components have finished
        staircaseComponents = [fixation, grating, key_resp_2]
        ...
        while continueRoutine:
            ...
            if grating.status == NOT_STARTED and tThisFlip >= 0.3-frameTolerance:
                # keep track of start time/frame for later
        ...
        for thisComponent in staircaseComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('staircase.stopped', globalClock.getTime(format='float'))
        ...
        # store data for scase (StairHandler)
        scase.addResponse(key_resp_2.corr, level)
        scase.addOtherData('key_resp_2.rt', key_resp_2.rt)
    ...
    # --- Prepare to start Routine "salutations" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('salutations.started', globalClock.getTime(format='float'))
    # keep track of which components have finished
    salutationsComponents = [text_5]
    for thisComponent in salutationsComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "salutations" ---
    for thisComponent in salutationsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('salutations.stopped', globalClock.getTime(format='float'))
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
