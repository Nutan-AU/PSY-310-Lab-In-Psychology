...
expName = 'WORD_PRIMING_EXPERIMENT_AU2220019_NUTAN_KAIN'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Lenovo\\Documents\\MON 2024-25\\FULL_TERM\\PSY-310_LAB_IN_PSYCHOLOGY\\7-PRIMING_TASK\\WORD_PRIMING_EXPERIMENT_AU2220019_NUTAN_KAIN_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    say_hello = visual.TextStim(win=win, name='say_hello',
        text='Hello!',
        font='Monospace',
    ...
    guide = visual.TextStim(win=win, name='guide',
        text='This Task involves two phases. \n\nIn the first phase, you will be shown a word and a scale is present below it. The two ends of the scale are marked as "familiar" or "unfamiliar" . With the help of a mouse, click over the scale, showcasing how much you perceive the word to be familiar or unfamiliar. \n\nThe second phase involves you to utlise the keyboard. In this, you will be shown a word with a few letters missing. Your task is to type the entire word and then submit it. \n\nIf you would like to participate, please click on SPACEBAR \nIf you would like to withdraw, please click on ESC',
        font='Monospace',
    ...
    study_text = visual.TextStim(win=win, name='study_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    study_slider = visual.Slider(win=win, name='study_slider',
        startValue=None, size=(1.0, 0.1), pos=(0, -0.3), units=win.units,
        labels=["Familiar","Unfamiliar"], ticks=(1, 2, 3, 4, 5), granularity=0.0,
        style='rating', styleTweaks=(), opacity=None,
        labelColor='LightGray', markerColor=[0.9608, 0.8431, 0.6863], lineColor=[0.6078, -0.2784, -0.2784], colorSpace='rgb',
        font='Monospace', labelHeight=0.05,
    ...
    test_textbox = visual.TextBox2(
         win, text=None, placeholder='Type here...', font='Monospace',
    ...
    ty = visual.TextStim(win=win, name='ty',
        text='Thank You for Your Partcipation. \nThe experiment has now ended. ',
        font='Monospace',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    while continueRoutine and routineTimer.getTime() < 2.5:
        ...
        # if say_hello is starting this frame...
        if say_hello.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            say_hello.frameNStart = frameN  # exact frame index
            say_hello.tStart = t  # local t and not account for scr refresh
            say_hello.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(say_hello, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'say_hello.started')
            # update status
            say_hello.status = STARTED
            say_hello.setAutoDraw(True)
        
        # if say_hello is active this frame...
        if say_hello.status == STARTED:
            # update params
            pass
        
        # if say_hello is stopping this frame...
        if say_hello.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > say_hello.tStartRefresh + 2.5-frameTolerance:
                # keep track of stop time/frame for later
                say_hello.tStop = t  # not accounting for scr refresh
                say_hello.tStopRefresh = tThisFlipGlobal  # on global time
                say_hello.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'say_hello.stopped')
                # update status
                say_hello.status = FINISHED
                say_hello.setAutoDraw(False)
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    guidelinesComponents = [guide, key_resp]
    ...
    while continueRoutine:
        ...
        # if guide is starting this frame...
        if guide.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'guide.started')
            # update status
            guide.status = STARTED
            guide.setAutoDraw(True)
        
        # if guide is active this frame...
        if guide.status == STARTED:
            # update params
            pass
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('STUDY_PHASE_WORDS_AU2220019.xlsx'),
    ...
    for thisTrial in trials:
        ...
        study_slider.reset()
        # keep track of which components have finished
        study_phaseComponents = [study_text, study_slider]
        ...
        while continueRoutine:
            ...
            if study_text.status == STARTED:
                ...
                if tThisFlipGlobal > study_text.tStartRefresh + 8.0-frameTolerance:
                    ...
                    study_text.tStopRefresh = tThisFlipGlobal  # on global time
            ...
            # *study_slider* updates
            
            # if study_slider is starting this frame...
            if study_slider.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                study_slider.frameNStart = frameN  # exact frame index
                study_slider.tStart = t  # local t and not account for scr refresh
                study_slider.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(study_slider, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'study_slider.started')
                # update status
                study_slider.status = STARTED
                study_slider.setAutoDraw(True)
            
            # if study_slider is active this frame...
            if study_slider.status == STARTED:
                # update params
                pass
            
            # Check study_slider for response to end Routine
            if study_slider.getRating() is not None and study_slider.status == STARTED:
                continueRoutine = False
        ...
        trials.addData('study_slider.response', study_slider.getRating())
        trials.addData('study_slider.rt', study_slider.getRT())
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('TEST_PHASE_WORDS_AU2220019.xlsx'),
    ...
    for thisTrial_2 in trials_2:
        ...
        # keep track of which components have finished
        test_phaseComponents = [test_text, test_textbox, submit_text, test_mouse]
        ...
        trials_2.addData('test_mouse.corr', test_mouse.corr)
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        if ty.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > ty.tStartRefresh + 4.0-frameTolerance:
                # keep track of stop time/frame for later
